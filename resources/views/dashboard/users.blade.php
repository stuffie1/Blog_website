...
@section('content')
    <h1 class="text-3xl font-bold ms-8 mt-3">Users</h1>
    <p class="ms-8 mb-12 text-indigo-500 ">Manage user account ,roles and settings</p>
    ...
    <form class="flex justify-between items-center mx-8 mb-4 columns-9" action="{{ route('myblogs.search') }}">
        <input type="text" name="filter"
            class="bg-gray-50 border columns-9 border-gray-300 text-gray-700 text-md ps-6 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-10"
    ...
    <a href="{{route('users.export')}}"
        class="text-white ms-3 h-10  bg-green-600 hover:bg-green-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 my-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">
        Export users

    </a>
    </div>
    @if ($users == [])
        <h1 class="text-3xl ms-96 text-gray-400">Theire no user yet</h1>
    @else
            ...
            <tbody>
                @foreach ($users as $user)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td scope="row"
                            class=" py-4 flex justify-center text-gray-900 whitespace-nowrap dark:text-white">
                        ...
                        <td class="text-sm py-4">
                           <span class="px-6 py-2  bg-slate-100  rounded-3xl"> {{ $user->role }}</span>
                        </td>
                        <td class="text-sm py-4">
                            {{ $user->created_at }}
                        </td>
...
@section('pagination')
    @if (!empty($users))
        <div style="width: 50%" class="mx-auto my-5">{{ $users->links() }}</div>
    @endif
@endsection
